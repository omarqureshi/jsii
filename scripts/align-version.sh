...
# The root resolutions entry pins the compiler fork with an any-version
# range (npm:@omarqureshi/jsii@>=0.0.0-0); the 0.0.0 there is range syntax,
# not an unaligned version marker — filter it out.
if find . -name package.json | grep -v node_modules | xargs grep "[^0-9]${marker}" | grep -v 'npm:@omarqureshi/jsii@>='; then
